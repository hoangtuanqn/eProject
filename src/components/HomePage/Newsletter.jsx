...
export default function Newsletter() {
    ...
    const checkEmailExists = async (email) => {
        const checkResponse = await fetch("https://679c74f887618946e65240bb.mockapi.io/api/v1/news");
        const existingEmails = await checkResponse.json();
        return existingEmails.some((entry) => entry.email === email);
    };

    const subscribeEmail = async (email) => {
        const response = await fetch("https://679c74f887618946e65240bb.mockapi.io/api/v1/news", {
            method: "POST",
            headers: {
            ...
            <form className="footer__form" onSubmit={handleSubmit}>
                <div className="footer__form-group">
                    ...
                    <button type="submit" className="footer__submit" disabled={loading}>
                        {loading ? <img src="/assets/icon/loading.gif" className="footer__loading" /> : "SUBSCRIBE"}
                    </button>
                </div>
            </form>
